...
public class EnumConverter extends Converter<Enum, JsonElement>
{
    ...
    public static int getEnumInt(final Enum clazz) {
        return ((Enum[])clazz.getClass().getEnumConstants()).length;
    }
    ...
    public static Enum setEnumInt(final Enum clazz, final int id) {
        for (int i = 0; i < ((Enum[])clazz.getClass().getEnumConstants()).length; ++i) {
            final Enum e = ((Enum[])clazz.getClass().getEnumConstants())[i];
            if (i == id) {
                return e;
    ...
    public static String[] getNames(final Enum clazz) {
        return Arrays.stream(clazz.getClass().getEnumConstants()).map((Function<? super Enum, ?>)Enum::name).toArray(String[]::new);
    }
